...
LOAD"sidbgm",8
RUN
# SIDBGM_SFX.prg (= BGM + voice 2 SFX overlay) も同じ流儀、PRG 名を sidbgmfx に変えるだけ:
c1541 -format "bgmfx,01" d64 disks/sidbgmsfx.d64 \
      -write examples/c64/SIDBGM_SFX.prg sidbgmfx \
      -write /path/to/your_tune.sid music
x64sc -8 disks/sidbgmsfx.d64
LOAD"sidbgmfx",8
RUN
```
...
`examples/c64/MMAPVIC.SL` (= mmap → vic_setmode → memcpy で screen RAM 直接書込):

```sl
#INCLUDE "C64_MEMMAP.LIB"
#INCLUDE "C64_VIC.LIB"

ARRAY BYTE GREETING[] = { "MMAPVIC DEMO" };   // 12 char + NUL

MAIN()
BEGIN
    MMAP_TRAMPOLINE();
    MMAP_SET(MMAP_NO_BASIC);    // I/O visible 維持
    VIC_SETMODE(VICM_TEXT, VIC_SCREEN_DEFAULT, VIC_FONT_ROM);
    MEMCPY(VIC_SCREEN_DEFAULT, GREETING, 12);   // 画面左上に表示
    INKEY(1);
END;
```
