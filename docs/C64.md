...
**runtime 構成**: `runtime/c64/slang_runtime.{h,c}` (I/O + 数学 + ビット) + `runtime/c64/slang_sprite.{h,c}` (VIC sprite bridge + VSYNC) + `runtime/c64/slang_joystick.{h,c}` (joystick bridge) + `runtime/c64/slang_kio.{h,c}` (KERNAL file I/O bridge) + `runtime/c64/slang_sid.{h,c}` (SID register direct + 単発 SFX + HVSC .sid BGM bridge) + `runtime/c64/slang_sidfx.{h,c}` (oscar64 `audio/sidfx` priority SFX overlay bridge) + `runtime/c64/slang_memmap.{h,c}` (oscar64 `c64/memmap.h` 経由 mmap 切替 bridge) + `runtime/c64/slang_vic.{h,c}` (oscar64 `c64/vic.h` 経由 VIC mode / bank 切替 bridge) + `runtime/c64/slang_mem.{h,c}` (oscar64 `string.h` memcpy / memset bridge)。slang_runtime.h は slang_sprite.h / slang_joystick.h / slang_kio.h / slang_sid.h / slang_sidfx.h / slang_memmap.h / slang_vic.h / slang_mem.h を chain include しているため、生成 C 側 extern と bridge 実装の signature drift が発生しません。
