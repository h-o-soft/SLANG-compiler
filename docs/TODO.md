...
#### T2b. ユーザー変数とシステム変数のラベル衝突防止
- `VAR A;`で`_A`ラベルが生成されるが、`^A`のエイリアス`_A EQU (_AF+1)`と衝突
- ユーザー変数`_A`とシステム変数`_A`が__WORK__内で二重定義になる
- 一般的な対策: ユーザー変数にプレフィックス`__`を使い(`__A`)、システム変数は`_`のまま残す等で名前空間を分離
- 旧実装はシンボル番号化(`__SYMxxx`)で回避していたが、可読性が犠牲
- 対象: SemanticAnalyzer.cs(ラベル生成), IrGenerator.cs(変数ラベル化), CodeGenerator.cs(__WORK__出力)
- 工数: 中（ラベル命名規則の統一設計が必要）
