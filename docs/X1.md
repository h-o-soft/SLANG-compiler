...
## Arkos Tracker music driver (AKG)

[Arkos Tracker](https://www.julien-nevo.com/arkostracker/) で作成した楽曲 (`.akg`) / 効果音バンク (`.akx`) を X1 の PSG (AY-3-8910 相当) で再生する driver。 全 X1 系 env (`x1` / `sosx1` / `x1native` / `x1native_slfs`) で **1 ソースのまま**動作し、 BGM + SFX overlay の同時再生に対応する。 CTC があれば割り込み駆動 (= ~61Hz auto)、 なければ VSYNC polling に自動 fallback する。

driver 本体は Targhan/Arkos の X1 player (= `runtime/x1/PlayerAkg_x1.asm` 等、 MIT License) を RASM でビルドした生バイナリ。 SLANG からは `runtime/libx1_arkos_akg.asm` の MACHINE 関数経由で呼ぶ。

### アドレス固定の制約 (= 重要)

Arkos の `.akg` / `.akx` データは **export 時のアドレスに置く前提**でビルドされた生データ (= 任意アドレスに再配置できない)。 driver bin も jump table が absolute address 参照のため ORG 固定。 本 sample の配置:

| 物 | アドレス | 備考 |
|---|---|---|
| driver | `$8000` | RASM `-DDRIVER_ORG=0x8000` で同 ORG ビルド要 |
| BGM (`.akg`) | `$B000` | Arkos export アドレス |
| SFX (`.akx`) | `$C000` | 同上 |

driver の ORG を変える場合は RASM の `-DDRIVER_ORG` と SLANG 側の `CONST ASM ARKOS_AKG_BASE` を同値に揃える。 X1turbo / S-OS で ROM bank と衝突しないよう `$8000` 以上を推奨。

### MACHINE 関数 (= `libx1_arkos_akg.asm`)

| 関数 | 引数 | 説明 |
|---|---|---|
| `AKG_INIT(mode)` | mode: 0=polling / 1=CTC | driver 初期化 (= 事前に `AKG_SET_CTC_PORT` 要) |
| `AKG_SET_CTC_PORT(port, vec)` | CTC port / vector base | CTC mode 用、 parameter area 書込 + IM2 vector page 設定 |
| `AKG_BGM_PLAY(addr)` | 楽曲 data addr | BGM 再生開始 |
| `AKG_BGM_STOP()` / `AKG_BGM_PAUSE()` / `AKG_BGM_RESUME()` | — | BGM 制御 |
| `AKG_SFX_INIT(addr)` | SFX bank addr | 効果音バンク登録 |
| `AKG_SFX_PLAY(num, ch)` | SFX 番号 (1-based) / channel (0-2) | 効果音再生 |
| `AKG_SFX_STOP(ch)` | channel | 効果音停止 |
| `AKG_PSG_PROC()` | — | polling mode で VSYNC ごとに呼ぶ (= CTC mode 時は no-op) |
| `AKG_PSG_END()` | — | CTC teardown + 停止 |
...
driver の RASM ビルド・サイズ定数生成・アドレス重なりチェック・各 env の asset 同梱を Makefile が担当する:

```sh
cd examples/X1_ARKOS
make x1            # → ARKOSMP_x1.d88      (LSX、 ndc で asset 同梱)
make sosx1         # → ARKOSMP_sosx1.d88   (S-OS、 HuDisk で asset 同梱)
make x1native      # → ARKOSMP_x1native.tap (cassette tape、 --tape-add 多段)
make x1native_slfs # → ARKOSMP_slfs.d88    (SLFS disk、 --slfs-add)
make all
```
...
`libx1_arkos_akg` と既存 `libx1_psg` (= MML driver) は同じ PSG を取り合うため、 同一プログラムでの併用は不可 (= 動的 link でどちらか使った方だけが binary に含まれる)。
