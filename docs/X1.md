...
WLA-DX は **PATH の通った場所に `wla-z80` / `wlalink` があれば OK** (Makefile は `WLA ?= wla-z80` で PATH を参照する)。banjo X1 サウンドを使う場合のみ必要で、 全員に必須のツールではない。`make setup-tools` は WLA-DX の有無を確認するが、 未導入でも導入コマンド例を表示して **setup は続行する** (システムへ勝手にインストールはしない)。
