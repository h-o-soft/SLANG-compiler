...
def main(argv=None):
    ...
    parser.add_option("", '--ay-master-clock', dest='ay_master_clock', default="3579545",
                      help='AY master clock for baked tone/envelope periods (default: 3579545; X1 PSG: 4000000)')

    (options, args) = parser.parse_args()

    global ay_master_clock
    try:
        ay_master_clock = float(options.ay_master_clock)
    except ValueError:
        parser.error("--ay-master-clock must be a number")
    ...
    if not options.sdas:
        outfile.write("; AY master clock used: " + str(ay_master_clock) + "\n")
