...
public sealed class X1Program
{
    ...
    public static X1Program DecodeFromBits(byte[] bits)
    {
        ...
        var infoSync = X1TapeFraming.FindNextSync(
            ...
            X1TapeFraming.InfoBlockMinSyncOnes);
        if (infoSync is null)
            throw new InvalidDataException("information block sync not found");

        var infoRaw = X1TapeFraming.ReadBytes(
            bits, infoSync.DataStartIndex, X1TapeFraming.InfoBlockTotalSize, out var afterInfo);
        ...
        ushort infoCkExpected = BinaryPrimitives.ReadUInt16BigEndian(
            infoRaw.AsSpan(X1InfoBlock.FieldsSize, 2));
        ushort infoCkActual = X1TapeFraming.ComputeChecksum(infoRaw.AsSpan(0, X1InfoBlock.FieldsSize));
        ...
        var dataRaw = X1TapeFraming.ReadBytes(
            bits, dataSync.DataStartIndex, dataLen + X1TapeFraming.ChecksumSize, out _);
    ...
    public TapFile Encode(uint sampleRate = 8000, string? tapeName = null)
    {
        ...
        BinaryPrimitives.WriteUInt16BigEndian(infoWithCk.AsSpan(X1InfoBlock.FieldsSize), infoCk);
        ...
        BinaryPrimitives.WriteUInt16BigEndian(dataWithCk.AsSpan(Data.Length), dataCk);
