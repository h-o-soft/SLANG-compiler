...
AKG_X1_SND_ISR_ENTRY:       dw 0    ; +$22 (0=direct vector, !=0=trampoline)
AKG_X1_SND_ISR_HANDLER_PTR: dw 0    ; +$24 (trampoline JP operand addr)
AKG_X1_SND_ISR_RETURN_MODE: db 0    ; +$26 (0=RETI, 1=RET)
...
AKG_X1_Init:
        ...
        ld a,(AKG_X1_SND_ISR_ENTRY)
        ld b,a
        ld a,(AKG_X1_SND_ISR_ENTRY+1)
        or b
        jr nz,AKG_X1_Init_Tramp
        ...
        dec hl
        ld bc,AKG_X1_PlayISR_RETI
        ld (hl),c
        inc hl
        ld (hl),b
        jr AKG_X1_Init_VectorDone
...
AKG_X1_Init_Tramp:
        ...
        ld (AKG_X1_CTC_Backup),bc      ; Save original vector
        dec hl
        ld bc,(AKG_X1_SND_ISR_ENTRY)
        ld (hl),c
        inc hl
        ld (hl),b

        ; Backup old handler ptr + write new (RETI or RET entry)
        ld hl,(AKG_X1_SND_ISR_HANDLER_PTR)
        ld c,(hl)
        inc hl
        ld b,(hl)
        ld (AKG_X1_ISR_Handler_Backup),bc
        dec hl
        ld a,(AKG_X1_SND_ISR_RETURN_MODE)
        or a
        jr nz,AKG_X1_Init_TrampRET
        ld de,AKG_X1_PlayISR_RETI
        jr AKG_X1_Init_TrampWrite
AKG_X1_Init_TrampRET:
        ld de,AKG_X1_PlayISR_RET
AKG_X1_Init_TrampWrite:
        ld (hl),e
        inc hl
        ld (hl),d

AKG_X1_Init_VectorDone:
...
AKG_X1_PlayISR_RET:
        ...
        ret
; ====================================================================================================
; PlayBody - 共通 body (= active/paused check + PLY_AKG_Play call、 通常 RET で終わる)
; ====================================================================================================
AKG_X1_PlayBody:
        di
        push af
        ld a,(AKG_X1_Active)
        or a
        jr z,AKG_X1_PlayBody_Skip
        ld a,(AKG_X1_Paused)
        or a
        jr nz,AKG_X1_PlayBody_Skip
        push hl
        push de
        push bc
        push ix
...
AKG_X1_PSG_END:
        ...
        ld hl,(AKG_X1_SND_ISR_HANDLER_PTR)
        ld de,(AKG_X1_ISR_Handler_Backup)
        ld (hl),e
        inc hl
        ld (hl),d
        ret
...
AKG_X1_ISR_Handler_Backup: dw 0    ; trampoline 旧 handler ptr (= SND_ISR_ENTRY != 0 時)
