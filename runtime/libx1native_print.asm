...
; x1native は cursor を (X,Y) = sXYADR で持つので AT_VRCALC で offset 化してから
; x1 (LSX) 版と同一の表示範囲判定を行う (= env 間で受理/拒否を完全一致させる)。
; stack を使わないので SP に一切触れない。
LD A, H
OR A
JR NZ, .tattr_ng
LD D, L                ; D = 属性バイト退避 (= AT_VRCALC は DE を保存する)
